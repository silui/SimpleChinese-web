...
function init() {
  ...
  imagesWidth = images.children.length * images.offsetWidth;

  for (var i = 0; i < images.children.length; i++) {
    images.children[i].style.width = images.offsetWidth + 'px';
  }

  images.style.width = imagesWidth + 'px';
  carousel.style.height = images.offsetHeight + 'px';
...
function animate() {
  ...
  var start = images.offsetLeft,
    end = index * carousel.offsetWidth * -1,
    offset = end - start;
  ...
  timer = setInterval(function () {
    ...
    images.style.left = offset / duration * t + start + 'px';
  }, interval);
}
...
function prevClick() {
  ...
  if (index < 0) {
    ...
    images.style.left = -index * imagesWidth + 'px';
  }
  animate();
}
...
function setImageWidth() {
  for (var i = 0; i < images.children.length; i++) {
    images.children[i].style.width = carousel.offsetWidth + 'px';
  }
}
